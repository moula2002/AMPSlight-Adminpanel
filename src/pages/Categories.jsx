...
export default function Categories() {
  ...
  return (
    <>
      <div className="max-w-6xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">Manage your categories</p>
          </div>
              ...
              <tbody>
                ...
                <tr key={item._id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                  <td className="py-4 px-4">
                    {item.imageUrl ? (
                      <img src={`http://localhost:5000${item.imageUrl}`} alt={item.name} className="w-14 h-14 object-cover rounded-xl border border-slate-100" />
                    ) : (
                  ...
                  <td className="py-4 px-4 text-sm text-slate-500 font-medium">{item.description}</td>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Add New Category" onSave={handleSave}>
        <div>
